...
#define fs_name(y)		_fs_name(ap143, y)
...
#	define CFG_ENV_ADDR		0x9f040000
#	ifdef COMPRESSED_UBOOT
#		define CONFIG_BOOTCOMMAND	"ping 192.168.1.1; nmrp; nor_two_part_fw_integrity_check 0x9f020000; bootm 0x9f020000"
#	else
#		define CONFIG_BOOTCOMMAND	"nmrp; nor_two_part_fw_integrity_check 0x9f680000; bootm 0x9f680000"
#	endif
#endif
...
#include <cmd_confdefs_hw29764648p4p0p32p2x2p0.h>
...
#define CFG_IMAGE_LEN   0x380000
#define CFG_IMAGE_BASE_ADDR (CFG_FLASH_BASE + 0x20000)
...
/* Solve bug #45160 on Shang-Hai Bugzilla */
#define WORKAROUND_QCA953X_GMAC_NMRP_HANG 1
...
/* These are WRITE-ONLY registers. You can only change them through
 * ath_reg_wr_nf() because ath_reg_wr() will read them after writing
 * values to them. */
#define GPIO_SET_ADDRESS	0x1804000c
#define GPIO_CLEAR_ADDRESS	0x18040010

#define RESET_BUTTON_GPIO	1   /* reset button (Input pin/Active LOW) */
#define WIFI_BUTTON_GPIO	2   /* WiFi button  (Input pin/Active LOW) */
#define WPS_BUTTON_GPIO		0   /* WPS button   (Input pin/Active LOW) */

#define ALL_BUTTONS_GPIO_MASK			\
		(1 << RESET_BUTTON_GPIO)	\
		| (1 << WIFI_BUTTON_GPIO)	\
		| (1 << WPS_BUTTON_GPIO)

/* for SIPO (Serial-In Parallel-Out) shift register LED controller */
#define LED_CLK_GPIO	16  /* SIPO shift register clock (Output pin/POSITIVE Edge triggered) */
#define LED_DATA_GPIO	15  /* SIPO shift register input (Output pin) */
#define LED_CLEAR_GPIO	14  /* SIPO shift register clear (Output pin/Active LOW) */
...
#define LED_CLK_DELAY_USEC	0

/*
 * LEDs definition. Lower number, earlier input into SIPO shift register LED
 * controller.
 */
#define POWER_GLED	0   /* power LED, green (Active LOW) */
#define STATUS_ALED	1   /* status LED, amber (Active LOW) */
#define WAN_IP_G_GLED	2   /* WAN IP got LED, green (Active LOW) */
#define WAN_IP_NG_ALED	3   /* WAN IP not get LED, amber (Active LOW) */
#define WLAN_LED	4   /* WLAN LED, blue(Active LOW) */
#define LAN1_100M_GLED	5   /* LAN port 1 100M LED, green (Active LOW) */
#define LAN1_10M_ALED	6   /* LAN port 1 10M LED, amber (Active LOW) */
#define LAN2_100M_GLED	7   /* LAN port 2 100M LED, green (Active LOW) */
#define LAN2_10M_ALED	8   /* LAN port 2 10M LED, amber (Active LOW) */
#define LAN3_100M_GLED	9  /* LAN port 3 100M LED, green (Active LOW) */
#define LAN3_10M_ALED	10   /* LAN port 3 10M LED, amber (Active LOW) */
#define LAN4_100M_GLED	11  /* LAN port 4 100M LED, green (Active LOW) */
#define LAN4_10M_ALED	12  /* LAN port 4 10M LED, amber (Active LOW) */
#define WPS_LED		13  /* WPS LED, green (Active LOW) */
