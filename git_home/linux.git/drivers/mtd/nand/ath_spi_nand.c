...
static struct nand_ecclayout ath_spi_nand_oob_64_wb = {
        .eccbytes       = 32,
 ...
 *      10              uncorrectable
 *      11              more than 4 bit errors in multiple pages
 */
static uint8_t ath_spi_nand_eccsr_wb(uint8_t status)
{
        return ((status >> 4 & 0x3) == 3) ? 2 : (status >> 4 & 0x3);
}
...
static void _ath_spi_program_load_wb(struct mtd_info *mtd, uint8_t *buf, int len, uint8_t *oob)
{
        int byte;
...
static int ath_spi_program_load_wb(struct mtd_info *mtd, uint8_t *buf, int len, uint8_t *oob)
{
        ...
        _ath_spi_program_load_wb(mtd, buf, len, oob);
...
static void *ath_spi_nand_priv_init(void)
{
	...
	for (option = 1; option >= 0; option--) {
		...
		for (i = 0; i < ARRAY_SIZE(ath_spi_nand_ids); i++) {
			...
			printk("Nand_Vendor: 0x%x\n", ath_spi_nand_ids[i].mfr);
	...
	case 0xc8:
		...
		break;
        case 0xef:
                priv->ecc_layout = &ath_spi_nand_oob_64_wb;
                priv->ecc_status = ath_spi_nand_eccsr_wb;
                priv->read_rdm_addr = ath_spi_read_rdm_addr_commom;
