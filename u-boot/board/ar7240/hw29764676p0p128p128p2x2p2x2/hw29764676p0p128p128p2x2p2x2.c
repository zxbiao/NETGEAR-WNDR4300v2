...
#include <config.h>
#include <version.h>
#include "ar7240_soc_hw29764676p0p128p128p2x2p2x2.h"
#include <nand.h>
...
/* control AR8035 LEDs */
#define ar8035_led_10_100m_override_init()                                   \
                phy_reg_write(0, 0, AR8035_LED_OVERRIDE_REG,                 \
                              (phy_reg_read(0, 0, AR8035_LED_OVERRIDE_REG) | \
                               (1 << AR8035_LED_10_100M_CTL)))
#define ar8035_led_10_100m_off()  do {                                       \
                ar8035_led_10_100m_override_init();                          \
                phy_reg_write(0, 0, AR8035_LED_OVERRIDE_REG,                 \
                              (phy_reg_read(0, 0, AR8035_LED_OVERRIDE_REG) & \
                               ~(1 << AR8035_LED_10_100M)));                 \
} while (0)
#define ar8035_led_10_100m_on()  do {                                        \
                ar8035_led_10_100m_override_init();                          \
                phy_reg_write(0, 0, AR8035_LED_OVERRIDE_REG,                 \
                              (phy_reg_read(0, 0, AR8035_LED_OVERRIDE_REG) | \
                               (1 << AR8035_LED_10_100M)));                  \
...
//	ar7240_reg_wr(AR9344_GPIO_OUT_FUNCTION5, (rddata & 0xffffffff)); /* 23, 22, 21, 20 */
	...
	ar7240_reg_rmw_clear(AR9344_GPIO_OE, (1 << WLAN_LED_GPIO)
	                    | (1 << TEST_LED_GPIO) | (1 << STATUS_LED_GPIO)
	                    | (1 << POWER_LED_GPIO)
	                    | (1 << xLNA_0_GPIO) | (1 << xLNA_1_GPIO) );
	/* let gpio pin of buttons be input enabled. */
	ar7240_reg_rmw_set(AR9344_GPIO_OE, (1 << WLAN_ON_OFF_SW_GPIO)
	                    | (1 << WPS_SW_GPIO)
	                    | (1 << RESET_BUTTON_GPIO));

	/* Set GPIO 18 and 19 as xLNA chain 0 and 1 respectively */
	ar7240_reg_rmw_clear(GPIO_OE_ADDRESS, (1 << 18) | (1 << 19));
	ar7240_reg_wr(GPIO_OUT_FUNCTION4_ADDRESS,
	              (ar7240_reg_rd(GPIO_OUT_FUNCTION4_ADDRESS)
	               & 0xffff0000) | 0x2f2e0000);
}
...
int	checkboard(args)
{
	board_str("U-boot dni29 V0.3 for DNI HW ID: 29764676 flash 128MB RAM 128MB 1st Radio 2x2 2nd Radio 2x2\n");
	return 0;
}
...
int do_ledctl (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	if (val == 0) {
		...
		ar9344_gpio_clear(TEST_LED_GPIO);
	} else if(val == 2) {
		ar9344_status_led_on();
		ar8035_led_10_100m_on();
	}
	return 0;
}
...
U_BOOT_CMD(
		...
		"ledctl	- Test LEDs\n",
		"\nledctl 1: turn on LEDs set 1\n"
		"\nledctl 2: turn on LEDs set 2\n"
		"\nledctl  : turn off all LEDs\n"
		);

static int wps_button_is_press(void)
{
	if ( ar7240_reg_rd(AR9344_GPIO_IN) & (1 << WPS_SW_GPIO))
		return 0;
	return 1;
}

static int wlan_button_is_pressed(void)
{
	if ( ar7240_reg_rd(AR9344_GPIO_IN) & (1 << WLAN_ON_OFF_SW_GPIO))
		return 0;
	return 1;
}
...
int do_button_test (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	if(board_reset_button_is_press())
		...
		printf("reset button is not pressed\n");

	if(wps_button_is_press())
		printf("WPS button is pressed\n");
	else
		printf("WPS button is not pressed\n");

	if(wlan_button_is_pressed())
		printf("WLAN button is pressed\n");
	else
		printf("WLAN button is not pressed\n");
