...
/* LED GPIO functions */
#define ath_gpio_set(_pin)    ath_reg_wr_nf(GPIO_SET_ADDRESS, (1 << _pin))
#define ath_gpio_clear(_pin)  ath_reg_wr_nf(GPIO_CLEAR_ADDRESS, (1 << _pin))
#define ath_power_led_on()    ath_gpio_clear(GPIO_PWR_GLED)
#define ath_power_led_off()   ath_gpio_set(GPIO_PWR_GLED)
#define ath_status_led_on()   ath_gpio_clear(GPIO_PWR_ALED)
#define ath_status_led_off()  ath_gpio_set(GPIO_PWR_ALED)
...
int	checkboard(args)
{
	board_str("U-boot dni29 V0.1 for DNI HW ID: 29764733 flash 4MB RAM 32MB 1st Radio 2x2\n");
	return 0;
}
